...
使用这份 Skill 时，优先遵循 xbatis 的原生表达方式，不要把它当成其他 ORM 或传统 XML MyBatis 使用。
...
- 第一件事就是从 GitHub / Gitee 下载 xbatis 源码到本地目录
...
- 使用 `@TableId`
- 仅在必要时使用 `@TableField`
- 项目推荐启用 Lombok，并在实体类上优先补 `@FieldNameConstants`
...
- 业务 DAO 注入 Mapper 时，优先重写 `setMapper(mapper)` 的方式完成注入，不要把 Mapper 通过构造方法一路往父类传
- 业务 DAO 需要补 Spring、Solon 等容器框架的自动注入注解，并跟随项目既有装配方式
...
24. DAO 注入仍以构造方法传 Mapper 为主，而不是按项目约定重写 `setMapper(mapper)` 并补 Spring、Solon 等容器自动注入注解
...
23. 创建时间、修改时间是否优先使用 `@TableField(defaultValue = "{NOW}", update = false)` 和 `@TableField(updateDefaultValue = "{NOW}", updateDefaultValueFillAlways = true)`
24. QO 是否优先配合 `@ConditionTarget` 体系，并在 `where()` 中使用 `WhereUtil.where(this)`
25. DAO 是否通过重写 `setMapper(mapper)` 注入 Mapper，并补 Spring、Solon 等容器自动注入注解
...
3. 修改时间字段优先使用 `@TableField(updateDefaultValue = "{NOW}", updateDefaultValueFillAlways = true)`
4. 使用 `@OnInsert`
5. 使用 `@OnUpdate`
...
16. 实体普通字段默认不写 `@TableField`；创建时间优先 `@TableField(defaultValue = "{NOW}", update = false)`，修改时间优先 `@TableField(updateDefaultValue = "{NOW}", updateDefaultValueFillAlways = true)`
17. 实体类只承载数据库表字段，禁止混入非数据库表字段；VO、QO、Model 中需要忽略的非数据库操作字段优先使用 `@Ignore` 或 `@Ignores`
18. 项目推荐启用 Lombok，并优先使用 `@FieldNameConstants` 和 `SysUser.Fields.id` 这类字段引用
19. `.as(...)` 优先使用 getter / 字段引用形式，例如 `.select(SysUser::getId, c -> c.as(SysUser::getId))`；只有特殊情况才使用 `.as("id")`
20. DAO 注入优先重写 `setMapper(mapper)` 并补 Spring、Solon 等容器自动注入注解，不默认走构造方法传 Mapper
...
25. XML 是补充手段，不是默认起点
26. 单 Mapper 模式优先统一 `XbatisMapper extends BasicMapper` 并配置 `@MapperScan`
